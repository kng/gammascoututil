...
class GSProtocolHandlerVers3(GSProtocolHandler):
	_version_pc_regex = RE("Version ([0-9]\.[0-9]{2})[a-zA-Z0-9]+ " + RE.DECIMAL + " " + RE.GDECIMAL + " " + RE.GHEXADECIMAL + " ([0-9]{2})\.([0-9]{2})\.([0-9]{2}) ([0-9]{2}):([0-9]{2}):([0-9]{2})")
	...
	def initmode(self):
		...
		if (versionstr is None) or (versionstr[1] != "Standard"):
			...
			if versionstr is not None:
				match = GSProtocolHandlerVers3._version_v1_regex.match(versionstr[1])
	...
	def readonlinevalue(self):
		...
		if line is not None:
			result = GSProtocolHandlerVers3._online_regex.match(line)
			if result:
	...
	def getversion(self):
		...
		elif GSProtocolHandlerVers3._version_pc_regex.match(versionstr):
			result["Mode"] = GSProtocolHandler.MODE_PC
			result["version"] = GSProtocolHandlerVers3._version_pc_regex[1]
			result["serial"] = int(GSProtocolHandlerVers3._version_pc_regex[2])
			result["buffill"] = int(GSProtocolHandlerVers3._version_pc_regex[3], 16)
			day = int(GSProtocolHandlerVers3._version_pc_regex[4])
			mon = int(GSProtocolHandlerVers3._version_pc_regex[5])
			year = int(GSProtocolHandlerVers3._version_pc_regex[6]) + 2000
			hour = int(GSProtocolHandlerVers3._version_pc_regex[7])
			mint = int(GSProtocolHandlerVers3._version_pc_regex[8])
			sec = int(GSProtocolHandlerVers3._version_pc_regex[9])
	...
	def readlog(self):
		...
		while True:
			...
			calcchksum = GSProtocolHandlerVers3._linechecksum(logdata)
